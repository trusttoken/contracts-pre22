  ...
  describe('TimeLock', () => {
    ...
    context('Transfers', () => {
      ...
      it('can transfer unlocked funds', async () => {
        ...
        await trustToken.connect(saftHolder).transfer(fourthAccount.address, parseTRU(100).div(8))
      ...
      it('if account has received tokens in normal way, they are transferable', async () => {
        ...
        await trustToken.connect(saftHolder).transfer(fourthAccount.address, parseTRU(10))

        expect(await trustToken.balanceOf(saftHolder.address)).to.equal(parseTRU(100))
        expect(await trustToken.balanceOf(fourthAccount.address)).to.equal(parseTRU(10))
      })
      ...
      it('if account has received tokens in normal way, they are transferable after some epochs has passed', async () => {
        ...
        await trustToken.connect(saftHolder).transfer(fourthAccount.address, parseTRU(35))

        expect(await trustToken.balanceOf(saftHolder.address)).to.equal(parseTRU(75))
        expect(await trustToken.balanceOf(fourthAccount.address)).to.equal(parseTRU(35))

        await expect(trustToken.connect(saftHolder).transfer(fourthAccount.address, 1)).to.be.revertedWith('attempting to transfer locked funds')
      })

      it('cannot transfer more than balance', async () => {
        await expect(trustToken.connect(saftHolder).transfer(fourthAccount.address, parseTRU(100).add(1))).to.be.revertedWith('insufficient balance')
      })
      ...
      describe('transferFrom', () => {
        ...
        it('can transfer unlocked funds', async () => {
          await timeTravel(provider, DAY * 120)
          await trustToken.connect(timeLockRegistry).transferFrom(saftHolder.address, fourthAccount.address, parseTRU(100).div(8))
        ...
        it('if account has received tokens in normal way, they are transferable', async () => {
          ...
          await trustToken.connect(timeLockRegistry).transferFrom(saftHolder.address, fourthAccount.address, parseTRU(10))
        ...
        it('if account has received tokens in normal way, they are transferable after some epochs has passed', async () => {
          ...
          await trustToken.connect(timeLockRegistry).transferFrom(saftHolder.address, fourthAccount.address, parseTRU(35))
        ...
        it('cannot transfer more than balance', async () => {
          await expect(trustToken.connect(timeLockRegistry).transferFrom(saftHolder.address, fourthAccount.address, parseTRU(100).add(1))).to.be.revertedWith('insufficient balance')
        })
      })
    })
  })
