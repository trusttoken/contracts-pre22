...
describe('TrueFiPool', () => {
  ...
  describe('borrow-repay', () => {
    ...
    it('reverts if borrower is not a lender', async () => {
      await expect(pool2.borrow(parseEther('1001'), 0)).to.be.revertedWith('CurvePool: Only lender can borrow')
    })
    ...
    it('when trueCurrency balance is not enough, withdraws from curve', async () => {
      ...
      await pool2.connect(borrower).borrow(parseEther('6000000'), 0)
    ...
    it('adds fee to claimableFees and borrows less if fee is not 0', async () => {
      ...
      await pool2.connect(borrower).borrow(borrowedAmount, 25)
