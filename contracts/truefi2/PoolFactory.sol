...
contract PoolFactory is IPoolFactory, UpgradeableClaimable {
    ...
    function createPool(address token) external onlyAllowedTokens(token) onlyNotExistingPools(token) {
        ...
        ITrueFiPool2(address(proxy)).initialize(ERC20(token), ITrueLender2Deprecated(address(0)), ftlAgency, safu, loanFactory, this.owner());
    ...
    function setFixedTermLoanAgency(IFixedTermLoanAgency _ftlAgency) external onlyOwner {
        require(address(_ftlAgency) != address(0), "PoolFactory: FixedTermLoanAgency address cannot be set to 0");
        ftlAgency = _ftlAgency;
        emit FixedTermLoanAgencyChanged(ftlAgency);
    }
