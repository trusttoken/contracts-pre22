...
contract TrueRewardBackedToken is CompliantDepositTokenWithHook {
    ...
    function enableTrueReward() external {
        ...
        uint yTUSDAmount = IFinancialOpportunity(iEarnInterfaceAddress()).deposit(msg.sender, balance);
        // emit some event
    ...
    function disableTrueReward() external {
        ...
        _disableIearn();
        uint yTUSDAmount = IFinancialOpportunity(iEarnInterfaceAddress()).withdrawAll(msg.sender);
    ...
    function totalSupply() public view returns (uint256) {
        if (totalIearnSupply() != 0) {
            uint ratio = IFinancialOpportunity(iEarnInterfaceAddress()).perTokenValue();
    ...
    function _transferAllArgs(address _from, address _to, uint256 _value) internal {
        ...
        if (senderTrueRewardEnabled) {
            ...
            uint yTUSDAmount = IFinancialOpportunity(iEarnInterfaceAddress()).withdrawTo(_from, _to, _value);
            _totalIearnSupply = _totalIearnSupply.sub(yTUSDAmount);
        ...
        if (receiverTrueRewardEnabled && !senderTrueRewardEnabled) {
            ...
            uint yTUSDAmount = IFinancialOpportunity(iEarnInterfaceAddress()).deposit(_to, _value);
            _totalIearnSupply = _totalIearnSupply.sub(yTUSDAmount);
