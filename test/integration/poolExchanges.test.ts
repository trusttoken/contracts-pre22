...
import { TrueFiPool__factory, TrueUsd__factory } from 'contracts'
...
describe('Pool exchanges', () => {
  it('sells all available CRV with 1inch', async () => {
    ...
    const tusd = TrueUsd__factory.connect('0x0000000000085d4780B73119b644AE5ecd22b376', pool.signer)
    const crv = TrueUsd__factory.connect('0xD533a949740bb3306d119CC777fa900bA034cd52', pool.signer)
    const crvBalanceBefore = await crv.balanceOf(pool.address)
    const dataUrl = `https://api.1inch.exchange/v2.0/swap?disableEstimate=true&fromTokenAddress=0xD533a949740bb3306d119CC777fa900bA034cd52&toTokenAddress=0x0000000000085d4780B73119b644AE5ecd22b376&amount=${crvBalanceBefore.toString()}&fromAddress=0xa1e72267084192Db7387c8CC1328fadE470e4149&slippage=2`
