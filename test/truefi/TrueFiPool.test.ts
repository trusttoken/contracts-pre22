...
describe('TrueFiPool', () => {
  ...
  describe('poolValue', () => {
    ...
    it('price of loan tokens is added to pool value after loans were given', async () => {
      ...
      const loan1 = await new LoanTokenFactory(owner).deploy(token.address, trustToken.address, borrower.address, lender.address, lender.address, parseEth(1e6), dayInSeconds * 365, 1000)
      ...
      const loan2 = await new LoanTokenFactory(owner).deploy(token.address, trustToken.address, borrower.address, lender.address, lender.address, parseEth(1e6), dayInSeconds * 365, 1000)
  ...
  describe('join-exit', () => {
    ...
    it('returns a basket of tokens on exit', async () => {
      ...
      await lender.connect(borrower).fund(loan1.address)
      await timeTravel(provider, dayInSeconds * 182.5)
      const loan2 = await new LoanTokenFactory(owner).deploy(token.address, trustToken.address, borrower.address, lender.address, lender.address, parseEth(1e6), dayInSeconds * 365, 2500)
      await lender.connect(borrower).fund(loan2.address)
    ...
    describe('two stakers', () => {
      ...
      beforeEach(async () => {
        loan1 = await new LoanTokenFactory(owner).deploy(token.address, trustToken.address, borrower.address, lender.address, lender.address, parseEth(1e6), dayInSeconds * 365, 1000)
        await mockRatingAgency.mock.getResults.returns(0, 0, toTrustToken(1e6))
        await lender.connect(borrower).fund(loan1.address)
        await timeTravel(provider, dayInSeconds * 182.5)
        ...
        await pool.connect(borrower).join(parseEth(1e6))
        loan2 = await new LoanTokenFactory(owner).deploy(token.address, trustToken.address, borrower.address, lender.address, lender.address, parseEth(1e6), dayInSeconds * 365, 2500)
        await lender.connect(borrower).fund(loan2.address)
      })
  ...
  describe('liquidExit', () => {
    ...
    it('after loan approved, applies a penalty', async () => {
      const loan1 = await new LoanTokenFactory(owner).deploy(token.address, trustToken.address, borrower.address, lender.address, lender.address, amount.div(3), dayInSeconds * 365, 1000)
