...
describe('TrueFiPool', () => {
  ...
  beforeEachWithFixture(async (wallets, _provider) => {
    ...
    mockRatingAgency = await deployMockContract(owner, TrueRatingAgencyJson)
    mockCurveGauge = await deployMockContract(owner, ICurveGaugeJson)
    mockCrv = await new MockErc20Token__factory(owner).deploy()
    const mockMinter = await deployMockContract(owner, ICurveMinterJson)
