...
describe('TrueRatingAgency', () => {
  ...
  beforeEachWithFixture(async (_wallets, _provider) => {
    ...
    await tusd.mint(owner.address, parseEther(toBigNumberString(1, 7)))
    ...
    await distributor.initialize(rater.address, trustToken.address, parseTRU(toBigNumberString(1, 7)))
    await rater.initialize(trustToken.address, distributor.address, mockFactory.address)

    await trustToken.mint(owner.address, parseTRU(toBigNumberString(1, 7)))
    await trustToken.mint(distributor.address, parseTRU(toBigNumberString(1, 7)))
    await trustToken.approve(rater.address, parseTRU(toBigNumberString(1, 7)))
  ...
  describe('Claim', () => {
    ...
    beforeEach(async () => {
      loanToken = await new LoanTokenFactory(owner).deploy(
        ...
        owner.address,
        owner.address,
        parseEther(toBigNumberString(5, 6)),
        monthInSeconds * 24,
        100,
      )

      await rater.setRewardMultiplier(rewardMultiplier)
      await tusd.approve(loanToken.address, parseEther(toBigNumberString(5, 6)))
      await rater.allow(owner.address, true)
      await submit(loanToken.address)
    })
    ...
    it('when called for the first time, moves funds from distributor to rater', async () => {
      ...
      expectCloseTo(balanceAfter.sub(balanceBefore), parseTRU(toBigNumberString(1, 5)))
    })
    ...
    it('when called for the first time, moves funds from distributor to rater (different reward multiplier)', async () => {
      ...
      await rater.claim(loanToken.address, owner.address, txArgs)
      const balanceAfter = await trustToken.balanceOf(rater.address)
      expectCloseTo(balanceAfter.sub(balanceBefore), parseTRU(toBigNumberString(5, 6)))
    })
    ...
    describe('Running', () => {
      ...
      it('properly saves claimed amount and moves funds (1 voter, called once)', async () => {
        await rater.yes(loanToken.address, 1000)
        await loanToken.fund()
        const expectedReward = parseTRU(toBigNumberString(5, 4)).mul(newRewardMultiplier)
      ...
      it('properly saves claimed amount and moves funds (1 voter, called multiple times)', async () => {
        ...
        await expectRoughTrustTokenBalanceChangeAfterClaim(expectedReward)
        await timeTravel(monthInSeconds * 12)
        expectedReward = parseTRU(toBigNumberString(5, 4)).mul(newRewardMultiplier)
        await expectRoughTrustTokenBalanceChangeAfterClaim(expectedReward)
      ...
      it('properly saves claimed amount and moves funds (multiple voters, called multiple times)', async () => {
        await rater.yes(loanToken.address, 2000)
        await trustToken.mint(otherWallet.address, parseTRU(toBigNumberString(1, 8)))
        await trustToken.connect(otherWallet).approve(rater.address, 3000)
        await rater.connect(otherWallet).yes(loanToken.address, 3000)
        await loanToken.fund()

        await timeTravel(monthInSeconds * 12)
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(2, 4)).mul(newRewardMultiplier), owner)
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(3, 4)).mul(newRewardMultiplier), otherWallet)
        await timeTravel(monthInSeconds * 6)
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(1, 4)).mul(newRewardMultiplier), owner)
        await timeTravel(monthInSeconds * 6)
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(1, 4)).mul(newRewardMultiplier), owner)
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(3, 4)).mul(newRewardMultiplier), otherWallet)
      })
    ...
    describe('Closed', () => {
      ...
      it('properly saves claimed amount and moves funds (multiple voters, called multiple times)', async () => {
        await trustToken.mint(otherWallet.address, parseTRU(toBigNumberString(1, 7)))
        await trustToken.connect(otherWallet).approve(rater.address, 3000)
        await rater.connect(otherWallet).yes(loanToken.address, 3000)
        await loanToken.fund()

        await timeTravel(monthInSeconds * 12)
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(2, 4)), owner)
        ...
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(6, 4)), otherWallet)
      })
      ...
      it('does not do anything when called multiple times', async () => {
        ...
        await expectRoughTrustTokenBalanceChangeAfterClaim(parseTRU(toBigNumberString(1, 5)), owner)
        await expectRoughTrustTokenBalanceChangeAfterClaim(0, owner)
      })
      ...
      it('does claim with withdraw', async () => {
        ...
        await expect(async () => rater.withdraw(loanToken.address, staked, txArgs))
          .to.changeTokenBalance(trustToken, owner, staked.add(parseTRU(toBigNumberString(1, 5))))
      })
    })
  })
})
