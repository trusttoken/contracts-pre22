...
export const setupTruefi2 = async (owner: Wallet, provider: MockProvider, customDeployed?: any) => {
  ...
  await rateModel.setBaseRateOracle(standardPool.address, standardBaseRateOracle.address)
  await rateModel.setBaseRateOracle(feePool.address, feeBaseRateOracle.address)
